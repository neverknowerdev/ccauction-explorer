...
export const auctions = pgTable('auctions', {
  ...
  floorPrice: numeric('floor_price', { precision: 38, scale: 18 }),
  currentClearingPrice: numeric('current_clearing_price', { precision: 38, scale: 18 }),
...
export const bids = pgTable('bids', {
  ...
  maxPrice: numeric('max_price', { precision: 38, scale: 18 }).notNull(),
  clearingPrice: numeric('clearing_price', { precision: 38, scale: 18 }),
...
export const clearingPriceHistory = pgTable('clearing_price_history', {
  ...
  auctionId: bigint('auction_id', { mode: 'number' }).notNull().references(() => auctions.id, { onDelete: 'cascade' }),
  time: timestamp('time', { withTimezone: true }).notNull(),
  clearingPrice: numeric('clearing_price', { precision: 38, scale: 18 }).notNull(),
  processedLogId: bigint('processed_log_id', { mode: 'number' }).references(() => processedLogs.id),
}, (table) => [
